...
use qres_core::adaptive::regime_detector::{RegimeDetector, RegimeChange};
...
fn test_regime_change_detection() {
    ...
    for i in 0..100 {
        ...
        assert_eq!(change, RegimeChange::None, "Should not detect drift in stable phase");
    }
    ...
    for _ in 0..10 {
        ...
        if let RegimeChange::Drift { current_error, threshold } = change {
            println!("Drift Detected! Error: {:.4}, Threshold: {:.4}", current_error, threshold);
            detected = true;
            break;
        }
    }
