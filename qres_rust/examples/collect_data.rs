...
fn calculate_features(data: &[u8]) -> Features {
    if data.is_empty() {
        return Features { entropy: 0.0, mean: 0.0, variance: 0.0, autocorr_1: 0.0 };
    }
    ...
    Features { entropy, mean, variance, autocorr_1 }
}
...
fn process_file(path: &str, output: &mut File) -> std::io::Result<()> {
    ...
    for chunk in buffer.chunks(64 * 1024) {
        ...
        writeln!(output, "{:.4},{:.4},{:.4},{:.4},{:.4},{:.4},{:.4},{:.4},{:.4}",
            features_header.entropy, features_header.mean, features_header.variance, features_header.autocorr_1,
...
fn main() -> std::io::Result<()> {
    ...
    writeln!(out_file, "entropy,mean,variance,autocorr_1,w_linear,w_simple,w_graph,w_spectral,w_lz")?;
