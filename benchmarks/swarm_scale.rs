...
async fn main() {
    ...
    println!("   Memory: {:.2} MB (Δ {:.2} MB)", active_mem as f64 / 1024.0 / 1024.0, delta_mem);
    ...
    if delta_mem < 100.0 {
        println!("\n🎉 SUCCESS: Total RAM impact ({:.2} MB) is minimal!", delta_mem);
        println!("   The swarm is GHOST-LIKE and production-ready!");
    } else {
        println!("\n⚠️  WARNING: RAM usage ({:.2} MB) is higher than expected.", delta_mem);
    }
