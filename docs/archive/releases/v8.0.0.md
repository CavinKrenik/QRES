...
This release consolidates the entire QRES ecosystem—Neural Intelligence, Swarm Persistence, and the Living Brain—into one solid, production-ready package. It represents the culmination of Phases 0-4.
...
- **Binary Fallback:** Smart spectral graph generation for non-UTF8 binary data in Tensor Mode.

### ⚛️ Tensor-Inspired Core
- **Tensor Networks:** High-dimensional state representation using Matrix Product States (MPS).
- **Tensor Simulation:** Noise modeling and AQC pruning (>40% efficiency gain).
...
- **State Broadcasting:** Real-time synchronization of state tensors across the WAN.
...
- `qres_tensor_cli.py` (Research Tool)
- `qres_tensor_receiver.py` (Swarm Node)
