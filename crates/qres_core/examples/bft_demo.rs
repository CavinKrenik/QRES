...
fn main() {
    ...
    for (i, v) in all_vectors.iter().enumerate() {
        let label = if i == 4 { "MALICIOUS ⚠️" } else { "Honest    ✓" };
        println!("   [{i}] {label}: [{:.2}, {:.2}]", 
                 v[0].to_num::<f32>(), v[1].to_num::<f32>());
    }

    // Calculate what naive Mean would produce
    let n = all_vectors.len() as f32;
    let mean: Vec<f32> = (0..2)
        .map(|i| all_vectors.iter().map(|v| v[i].to_num::<f32>()).sum::<f32>() / n)
        .collect();
    ...
    println!("   Krum (PROTECTED): [{:.2}, {:.2}]", krum_f32[0], krum_f32[1]);

    // Calculate difference
    let diff: f32 = mean.iter().zip(krum_f32.iter())
        .map(|(m, k)| (m - k).abs())
        .sum();
