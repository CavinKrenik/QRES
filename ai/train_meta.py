...
def tree_to_rust(node, depth):
    ...
    if tree.children_left[node] == tree.children_right[node]: # Leaf
        ...
        return f"{indent}{class_val}\n"
    else:
        threshold = tree.threshold[node]
        feat_idx = tree.feature[node]
        feat_name = feature_names[feat_idx]
        
        left = tree_to_rust(tree.children_left[node], depth + 1)
        right = tree_to_rust(tree.children_right[node], depth + 1)
